Describe 'The firewall' {
    ...
    Context 'should allow influxdb' {
        ...
        It 'on port 8083' {
            ($ufwOutput | Where-Object {$_ -match '(8083/tcp)\s*(ALLOW)\s*(Anywhere)'} ) | Should Not Be $null
        }
        ...
        It 'on port 8088' {
            ($ufwOutput | Where-Object {$_ -match '(8088/tcp)\s*(ALLOW)\s*(Anywhere)'} ) | Should Not Be $null
        }

        It 'on port 25826' {
            ($ufwOutput | Where-Object {$_ -match '(25826/tcp)\s*(ALLOW)\s*(Anywhere)'} ) | Should Not Be $null
        }
    }
