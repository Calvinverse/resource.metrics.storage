...
file "#{consul_template_config_path}/telegraf_influxdb_inputs.hcl" do
  ...
  content <<~HCL
    ...
    template {
      ...
      command = "chown #{node['telegraf']['service_user']}:#{node['telegraf']['service_group']} #{telegraf_config_directory}/inputs_influxdb.conf && systemctl restart #{telegraf_service}"
